...
//-----  properties ---------------------------------------------------------------------------------------------------
     ...
     *
     * @param context
     * @param layoutRessource Row-Layout-Ressource for the list item.
     * @param listData Array that holds the data of the list.
     */
    public ListCheckboxSampleAdapter(Context context, int layoutRessource, int[] targetViews, ArrayList<ListCheckboxSampleData> listData){
        super(context,layoutRessource,targetViews);
        mListData = listData;
        // we use a custom holder that does not implement PDEHolderInterface so turn automatic creation off.
        setAutomaticHolderCreation(false);
    }
    ...
    protected void initListItem(PDEListItem listItem){
        ...
        holder.checkboxBtn.addListener(this,"buttonCheckboxPressed", PDEAgentController.PDE_AGENT_CONTROLLER_EVENT_ACTION_WILL_BE_SELECTED);
    ...
    protected void fillListItem(PDEListItem listItem){
        ...
        holder.checkboxBtn.setText(((ListCheckboxSampleData) mListData.get(position)).getTitle());
        // restore checked state
        holder.checkboxBtn.setSelected(((ListCheckboxSampleData) mListData.get(position)).getChecked());
    ...
    public void createDummyElements(int num){
        ...
        for (i = 1; i <= num; i++) {
            ...
            dummy.setTitle(new String("Checkbox "+i));
    ...
    public void buttonCheckboxPressed(PDEEvent event) {
        ...
        elementData = (ListCheckboxSampleData) mListData.get(position);
